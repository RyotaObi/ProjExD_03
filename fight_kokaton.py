...
class Bomb:
    ...
    爆弾に関するクラス
    """
    def __init__(self): #赤い半径10の円
        """
        爆弾円Surfaceを生成する
        """
        rad = random.randint(10, 100) #円の半径ランダム
...
class Beam:
    ...
    def update(self, screen: pg.Surface):
        ...
        if not yoko:
            self.vx *= -1
            pg.transform.rotozoom(self.img, 90, 1.0) #ビーム反転
        if not tate:
            self.vy *= -1
            pg.transform.rotozoom(self.img, 90, 1.0) #ビーム反転
        self.rct.move_ip(self.vx, self.vy)
...
def main():
    ...
    bird = Bird(3, (900, 400))
    bombs = [Bomb() for i in range(NUM_OF_BOMBS)] #インスタンスがnum個並んだリスト
    beam = None
    score = Score()
